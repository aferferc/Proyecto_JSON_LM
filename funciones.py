...
def MediaStock(datos, modelo):
    unidades = []
    totalUnidades= 0
    for c in datos:
        for coche, info in c['inventario'].items():
            if coche.lower() == modelo.lower():
                unidades.append(info['stock'])
    for u in unidades:
        totalUnidades = totalUnidades + u
    media = totalUnidades / len(unidades)
    return media
       
def VerMediaStock(datos):
    modelo = optenerModelo(datos)
    media = MediaStock(datos, modelo)
    media = round(media)
    print('La media de unidades por concesionario del modelo ', modelo, ' es de ', media, ' unidades.')
        
        
def optenerCiudad(datos):
    ciudad = str(input('Introduzca la ciudad deseada: '))
    if comprobarCiudadEnRegistro(datos, ciudad):
        return ciudad
    else:
        print("Error: No se ha podido encontrar ningun concesionario registrado en esa ciudad")
        sys.exit(1)
        
def comprobarCiudadEnRegistro(datos, ciudad):
    for c in datos:
        ciudadRegistro = c['city']
        if ciudadRegistro.lower() == ciudad.lower():
            return True
    
def mostrarCochesEnRangoPrecioCiudad(datos):
    pmax, pmin = optenerPrecios()
    ciudad = optenerCiudad(datos)
    print('Los modelos en rango encontrados en la ciudad de ', ciudad, ' son:')
    for c in datos:
        ciudadRegistro = c['city']
        if ciudad.lower() == ciudadRegistro.lower():
            for coche, info in c['inventario'].items():
                if comprobarRangoPrecio(info['precio_base'], pmax, pmin):
                    print('Modelo: ', coche, ' | Precio Base: ', info['precio_base'], ' | Concesionario: ', c['name'])
